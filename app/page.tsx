...
export default function Home() {
  ...
  useEffect(() => {
    let value: { answers?: Record<number, number>; checked?: Record<number, boolean>; current?: number; started?: boolean; finished?: boolean; deadline?: number | null; startedAt?: number | null; timedOut?: boolean } | null = null;
    try {
      const saved = window.localStorage.getItem(`kangaroo-progress-${setId}`);
      if (saved) value = JSON.parse(saved) as typeof value;
    } catch { /* local progress is optional */ }
    queueMicrotask(() => {
      const progress = value as { answers?: Record<number, number>; checked?: Record<number, boolean>; current?: number; started?: boolean; finished?: boolean; deadline?: number | null; startedAt?: number | null; timedOut?: boolean } | null;
      if (progress) {
        const savedDeadline = typeof progress.deadline === "number" ? progress.deadline : null;
        const expired = Boolean(progress.started && savedDeadline && savedDeadline <= Date.now());
        setAnswers(progress.answers ?? {});
        setChecked(progress.checked ?? {});
        setCurrent(typeof progress.current === "number" ? progress.current : 0);
        setStarted(Boolean(progress.started));
        setFinished(Boolean(progress.finished) || expired || Boolean(progress.timedOut));
  ...
  useEffect(() => {
    ...
    try { window.localStorage.setItem(`kangaroo-progress-${setId}`, JSON.stringify({ answers, checked, current, started, finished, deadline, startedAt, timedOut })); } catch { /* ignore */ }
  }, [answers, checked, current, started, finished, deadline, startedAt, timedOut, setId, loadedSetId]);

  useEffect(() => {
    if (!deadline || !started || finished) return;
    const tick = () => {
  ...
  function goTo(index: number) { setCurrent(Math.max(0, Math.min(questions.length - 1, index))); setShowHint(false); setShowSolution(false); setShowScratch(false); }
  function begin() { const now = Date.now(); setStarted(true); setFinished(false); setTimedOut(false); setStartedAt(now); setDeadline(now + totalSeconds * 1000); setTimeLeft(totalSeconds); goTo(current); }
  function reset(start = false) { try { window.localStorage.removeItem(`kangaroo-progress-${setId}`); } catch { /* local progress is optional */ } setAnswers({}); setChecked({}); setCurrent(0); setStarted(start); setFinished(false); setTimedOut(false); setDeadline(null); setStartedAt(null); setTimeLeft(0); setShowHint(false); setShowSolution(false); setShowScratch(false); setScratch(""); setFilter("all"); }
  function startOver() { reset(true); }
  function switchSet(nextSetId: string) { if (nextSetId === setId) return; setSetId(nextSetId); reset(); }
