      ...
      <header className="editor-topbar">
        ...
        <div className="editor-top-actions">
          <label className="editor-set-picker">
            <span>Current question set</span>
            <select value={setId} onChange={(event) => changeSet(event.target.value)}>
              {setEntries.map((entry) => <option key={entry.id} value={entry.id}>{entry.year} · {entry.group} · {entry.grades}</option>)}
            </select>
          </label>
